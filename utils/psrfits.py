...
class PsrfitsFile(object):
    def __init__(self, psrfitsfn):
        if not os.path.isfile(psrfitsfn):
            raise ValueError("ERROR: File does not exist!\n\t(%s)" %
                                psrfitsfn)
        # self.filename =  os.path.basename(psrfitsfn)
        self.filename =  psrfitsfn
        #modify zhang
        self.fn, self.ext=os.path.splitext(os.path.basename(psrfitsfn))
        ...
        self.mjd=self.specinfo.mjd
        self.ra_str=self.specinfo.ra_str
        self.dec_str=self.specinfo.dec_str
        self.telescope=self.specinfo.telescope
        self.df = abs(self.freqs[1] - self.freqs[0])
    ...
    def read_subint(self, isub, apply_weights=True, apply_scales=True,
        ...
        if apply_scales or apply_offsets:
            ...
            if apply_offsets:
                ...
                # #wang covert2_to8
                # #by zhang 25.2.18
                # data += (self.get_offsets(isub)- np.float32(0.5) *self.get_scales(isub))
        if apply_weights:
            data = data.reshape((self.nsamp_per_subint * self.npoln, self.nchan))
    ...
    def get_spectra(self, startsamp, N):
        ...
        # Ensure endsub is within bounds and is an integer
        endsub = int(min(endsub, int(self.nsubints) - 1))
        
        trunc = int(((endsub + 1) * self.nsamp_per_subint) - (startsamp + N))
        ...
        return Spectra(freqs, self.tsamp, data,
                               starttime=self.tsamp*startsamp, dm=0)
    # modify by zhang      
    def get_spectra_slide(self, start_time, start_subint, end_subint):
        """Return data from PSRFITS file for the specified subintegration range.
        
        Args:
            start_time: Starting time in seconds
            start_subint: Starting subintegration index
            end_subint: Ending subintegration index
            
        Returns:
            Spectra object containing the requested data
        """
        if not self.ext.endswith('.fits'):
            raise ValueError("This method is for PSRFITS files only")
        
        # Ensure subint indices are within valid range
        start_subint = max(0, start_subint)
        end_subint = min(end_subint, self.nsubints - 1)
        
        # Calculate starting sample and number of samples
        startsamp = start_subint * self.nsamp_per_subint
        # Include all samples in the end subint
        endsamp = (end_subint + 1) * self.nsamp_per_subint
        N = min(endsamp - startsamp, self.nspec - startsamp)  # Ensure we don't read past the end
        
        # Get the spectra data using the existing method
        spec = self.get_spectra(startsamp, N)
    ...
    def get_spectra_all(self):
        """Return 2D array of all data from PSRFITS file.

        Output:
            data: Spectra object containing all data
        """
        # Read all subints
        data = []
        for isub in range(int(self.nsubints)):
            data.append(self.read_subint(isub))
    ...
    # modify by zhang
    def get_spectra_subint(self, subints):
        """Return 2D array of data from specified subints in PSRFITS file.
    
        Inputs:
            subints: list or array of subint indices to read
    
        Output:
            data: Spectra object containing data from specified subints
        """
        # Validate subint indices
        subints = np.array(subints)
        if np.any(subints < 0) or np.any(subints >= self.nsubints):
            raise ValueError(f"Subint indices must be between 0 and {self.nsubints-1}")
        
        # Read specified subints
        data = []
        for isub in subints:
            data.append(self.read_subint(isub))
        ...
        # Calculate start time based on first subint
        starttime = self.tsamp * self.nsamp_per_subint * subints[0]

        return Spectra(freqs, self.tsamp, data,
                            starttime=starttime, dm=0)
...
class SpectraInfo(object):
    def __init__(self, filenames):
        ...
        for ii, fn in enumerate(filenames):
            ...
            self.start_MJD[ii] = "%.11f" % (Decimal(primary['STT_IMJD']) +  Decimal((primary['STT_SMJD']+ \
